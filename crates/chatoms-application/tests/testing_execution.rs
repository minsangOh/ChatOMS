...
impl ValidationCommandExecutor for ScriptedExecutor {
    fn start_validation_command(
        ...
        self.observed
            .push((worktree_path.to_path_buf(), approval.kind));
        match self
            .scripted
            .pop_front()
...
fn begin_returns_approvals_ordered_by_the_fixed_kind_sequence_regardless_of_insertion_order() {
    ...
    let inputs = TestingBatchStarter::new(&mut repository)
        .begin(BeginTestingBatchRequest::new(task_id, 3))
...
fn begin_rejects_a_task_that_is_not_testing() {
    ...
    let error = TestingBatchStarter::new(&mut repository)
        .begin(BeginTestingBatchRequest::new(task_id, 3))
...
fn begin_rejects_a_stale_task_version() {
    ...
    let error = TestingBatchStarter::new(&mut repository)
        .begin(BeginTestingBatchRequest::new(task_id, 1))
