...
use chatoms_application::{
    context_package_review_execution::{
        BeginContextPackageReviewExecutionRequest, ContextPackageReviewExecutionInputs,
        ContextPackageReviewExecutionRecorder, ContextPackageReviewExecutionStarter,
    },
...
use chatoms_ports::{
    ...
    repository::{
        ContextPackageManifestRecord, GitIsolationStatus, ProjectRecord, ProviderConsent,
        ReviewResultOutcome, TaskBriefRecord, TaskGitIsolation,
    },
    review::{ReviewExecutionBrief, ReviewExecutionResult, ReviewExecutionStartOutcome},
};
...
/// `ContextPackageReviewExecutionStarter::begin` delegates to); every other
/// `GitService` method panics if called.
struct GitFake {
    verified: bool,
    calls: usize,
}
...
impl GitService for GitFake {
    fn is_available(&mut self) -> Result<bool, PortFailure> {
        unreachable!("not used by ContextPackageReviewExecutionStarter")
    }
...
impl ContextPackageReviewExecutor for ScriptedExecutor {
    fn start_review(
        &mut self,
        worktree: &Path,
...
fn context_package_review_pair(
    task_id: TaskId,
    ...
    (key, consent, manifest)
}

/// A task in `Reviewing` with a matching `WorktreeReady` isolation record, an
/// owning project, an attached `TaskBrief`, and an already-prepared
/// `ContextPackageV1` consent+manifest pair — ready for
/// `ContextPackageReviewExecutionStarter::begin` to succeed.
fn setup_reviewing_prepared(version: u64) -> (FakeRepository, TaskId) {
    ...
    let (key, consent, manifest) = context_package_review_pair(task_id, version, 200);
    repository.consents.insert(key, consent);
    repository.context_package_manifests.insert(key, manifest);
    repository.seed_task(task, history);
    (repository, task_id)
}

/// A task already in `Reviewing`, with no isolation/brief/readiness evidence
/// attached — sufficient for `ContextPackageReviewExecutionRecorder`, which
...
#[test]
fn begin_returns_worktree_brief_and_diff_when_every_precondition_holds_and_records_no_consent() {
    let (mut repository, task_id) = setup_reviewing_prepared(3);
    ...
    let consents_before = repository.consents.len();
    ...
    .begin(BeginContextPackageReviewExecutionRequest::new(task_id, 3))
    .expect("begin succeeds");

    assert_eq!(inputs.task.state, TaskState::Reviewing);
    assert_eq!(
        inputs.task.version, 3,
        "starting Context Package v1 Review drives no state transition"
    );
    ...
    assert_eq!(
        repository.consents.len(),
        consents_before,
        "begin must never create or reuse-write any consent — Prepare already recorded it"
    );
    assert!(
        !repository.calls.contains(&"save_review_consent"),
        "begin must never call save_review_consent"
    );
}
...
fn begin_rejects_unsupported_capability_with_no_execution_and_state_preserved() {
    ...
    let error = ContextPackageReviewExecutionStarter::new(
        &mut repository,
        &mut time,
        &mut capability,
        &mut git,
...
fn begin_rejects_when_task_is_not_reviewing() {
    ...
    repository.briefs.insert(task_id, brief_record(task_id));
    let (key, consent, manifest) = context_package_review_pair(task_id, 1, 200);
    repository.consents.insert(key, consent);
    repository.context_package_manifests.insert(key, manifest);
    repository.seed_task(task, history);
    let mut time = FakeTime::at(30);
    ...
    .begin(BeginContextPackageReviewExecutionRequest::new(task_id, 1))
    .expect_err("a Testing task cannot start Context Package v1 Review");
...
fn begin_rejects_a_stale_task_version() {
    let (mut repository, task_id) = setup_reviewing_prepared(5);
    let mut time = FakeTime::at(30);
    let mut capability = FakeCapability(ProviderCapabilityStatus::Supported);
    let mut git = GitFake {
        verified: true,
...
fn begin_rejects_missing_isolation_record_with_no_state_change() {
    ...
    repository
        ...
        .insert(project_id, project_record(project_id));
    repository.briefs.insert(task_id, brief_record(task_id));
    let (key, consent, manifest) = context_package_review_pair(task_id, 3, 200);
    repository.consents.insert(key, consent);
    repository.context_package_manifests.insert(key, manifest);
    repository.seed_task(task, history);
    let mut time = FakeTime::at(30);
...
#[test]
fn begin_rejects_when_the_context_package_v1_pair_is_not_prepared_and_never_reads_the_diff() {
    let (task, history) = restored_task(TaskState::Reviewing, 3, 20, None);
    let task_id = task.id();
    let project_id = task.project_id();
    let mut repository = FakeRepository::default();
    ...
    .begin(BeginContextPackageReviewExecutionRequest::new(task_id, 3))
    .expect_err("an unprepared Context Package v1 pair must be rejected");

    assert_eq!(error.code(), ApplicationErrorCode::InvalidState);
    assert_eq!(
        diff.called_with, None,
        "the diff must never be read before readiness is confirmed"
    );
}

#[test]
fn begin_rejects_a_no_changes_diff_with_no_state_change() {
    let (mut repository, task_id) = setup_reviewing_prepared(3);
    let mut time = FakeTime::at(30);
    let mut capability = FakeCapability(ProviderCapabilityStatus::Supported);
    let mut git = GitFake {
...
#[test]
fn begin_rejects_a_too_large_diff_with_no_state_change() {
    let (mut repository, task_id) = setup_reviewing_prepared(3);
    let mut time = FakeTime::at(30);
    let mut capability = FakeCapability(ProviderCapabilityStatus::Supported);
    let mut git = GitFake {
...
#[test]
fn begin_rejects_a_timed_out_diff_read_with_no_state_change() {
    let (mut repository, task_id) = setup_reviewing_prepared(3);
    let mut time = FakeTime::at(30);
    let mut capability = FakeCapability(ProviderCapabilityStatus::Supported);
    let mut git = GitFake {
...
#[test]
fn begin_rejects_an_uncertain_diff_read_with_no_state_change() {
    let (mut repository, task_id) = setup_reviewing_prepared(3);
    let mut time = FakeTime::at(30);
    let mut capability = FakeCapability(ProviderCapabilityStatus::Supported);
    let mut git = GitFake {
...
#[test]
fn begin_rejects_a_malformed_diff_port_error_with_no_state_change() {
    let (mut repository, task_id) = setup_reviewing_prepared(3);
    let mut time = FakeTime::at(30);
    let mut capability = FakeCapability(ProviderCapabilityStatus::Supported);
    let mut git = GitFake {
...
#[test]
fn context_package_review_execution_inputs_debug_output_never_reveals_the_diff_text() {
    let (mut repository, task_id) = setup_reviewing_prepared(3);
    let mut time = FakeTime::at(30);
    let mut capability = FakeCapability(ProviderCapabilityStatus::Supported);
    let mut git = GitFake {
...
fn run_and_record_completed_reaches_awaiting_user_diff_approval() {
    ...
    let view = ContextPackageReviewExecutionRecorder::new(&mut repository, &mut time)
        .run_and_record(
            task_id,
            4,
            WORKTREE_PATH,
...
#[test]
fn run_and_record_post_readiness_preflight_rejection_falls_back_to_recovery_required() {
    let (mut repository, task_id) = setup_reviewing_only(4);
    let mut time = FakeTime::at(40);
    ...
    let view = ContextPackageReviewExecutionRecorder::new(&mut repository, &mut time)
        ...
        .expect("a preflight rejection is still recorded, never left stuck");
...
fn panic_containment_does_not_report_success_when_the_recovery_write_is_itself_rejected() {
    ...
    let error = ContextPackageReviewExecutionRecorder::new(&mut repository, &mut time)
        .run_and_record_with_panic_containment(
            task_id,
...
#[test]
fn begin_then_run_and_record_connects_readiness_diff_and_result_end_to_end() {
    let (mut repository, task_id) = setup_reviewing_prepared(3);
    let mut time = FakeTime::at(30);
    let mut capability = FakeCapability(ProviderCapabilityStatus::Supported);
    let mut git = GitFake {
    ...
    let inputs: ContextPackageReviewExecutionInputs = ContextPackageReviewExecutionStarter::new(
        &mut repository,
        &mut time,
        &mut capability,
        &mut git,
