...
/// Mirrors `implementation_execution.rs`'s identically named sentinel:
/// proves the caught panic's payload never surfaces in anything this
/// recorder returns.
...
/// A task ready to activate Context Package v1 Implementation:
/// `AwaitingDesignApproval` state, a matching `WorktreeReady` isolation
...
fn begin_rejects_missing_isolation_record_with_no_state_change() {
    ...
    repository
        .planning_results
        .insert(task_id, completed_planning_result(task_id, "masked plan"));
    seed_context_package_implementation_pair(&mut repository, task_id, 3);
    repository.seed_task(task, history);
    let mut time = FakeTime::at(30);
    let mut capability = FakeCapability(ProviderCapabilityStatus::Supported);
...
fn run_and_record_success_reaches_testing() {
    ...
    let view = ContextPackageImplementationExecutionRecorder::new(&mut repository, &mut time)
        .run_and_record(
            task_id,
            4,
            "C:/managed/task",
...
fn run_and_record_post_transition_assembly_or_preflight_rejection_falls_back_to_recovery_required()
{
    ...
    // folded by `ClaudeImplementationAdapter`'s
    // `ContextPackageImplementationExecutor` impl into `PreflightRejected`
    // before this recorder ever sees it (see
    // `chatoms_infrastructure::claude_implementation`), so this scripted
    // outcome exercises exactly what this recorder observes for either
    // cause.
    ...
    let view = ContextPackageImplementationExecutionRecorder::new(&mut repository, &mut time)
        ...
        .expect("a post-transition rejection is still recorded, never left stuck");
...
fn panic_containment_does_not_report_success_when_the_recovery_write_is_itself_rejected() {
    ...
    let error = ContextPackageImplementationExecutionRecorder::new(&mut repository, &mut time)
        .run_and_record_with_panic_containment(
            task_id,
