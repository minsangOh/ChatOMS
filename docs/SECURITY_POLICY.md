...
- 에디터 억제는 `GIT_EDITOR=true`로 한다. 실제 Git(2.48.1, Windows)로 검증한 결과 `merge --continue`의 내부 commit 단계는 이 값이 없으면 Vim을 실행하며, adapter가 write에 사용하는 null stdin 위에서 Vim이 즉시 오류를 내지 않고 무한 대기해 write timeout 이전에 프로세스가 정리되지 않는 것을 실제 테스트로 확인했다. `GIT_EDITOR=true`의 실제 동작은 Git 공식 소스(`editor.c`/`run-command.c`)로 확인했다: Git은 editor 실행에 shell 사용 가능 플래그를 설정하지만 `true`에는 shell 메타문자가 없어 shell wrapper 없이 argv를 직접 실행하고, Windows Git은 이때 자체 PATH 탐색으로 `true.exe`를 찾는다(shell builtin이 아니다). Adapter가 child process PATH를 trusted Git 설치 트리의 `cmd`/`bin`/system directory로만 제한하고 task worktree·사용자 PATH·현재 CWD를 포함하지 않으므로, `GIT_EDITOR=true`는 항상 그 신뢰된 설치 트리 내부의 `mingw64\bin\true.exe`로만 resolve되어 항상 성공하며, 이미 준비된 `MERGE_MSG`를 그대로 사용해 exit 0으로 종료한다.
