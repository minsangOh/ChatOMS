...
describe("context package implementation readiness IPC", () => {
  it("calls the exact command name and accepts a well-formed ready response", async () => {
    ...
    await expect(
      client.getContextPackageImplementationReadiness("task-id", 6),
    ).resolves.toEqual({ ready: true });
    expect(transport).toHaveBeenCalledWith("get_context_package_implementation_readiness", {
      taskId: "task-id",
      expectedVersion: 6,
    });
  });
  ...
  it("preserves a safe precondition error without exposing internal fields", async () => {
    ...
    const error = await client
      .getContextPackageImplementationReadiness("task-id", 1)
